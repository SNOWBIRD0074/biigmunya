...
const config = {
    ...
    NEWSLETTER_JID: '120363402585439267@newsletter', // UPDATED CHANNEL ID
    NEWSLETTER_MESSAGE_ID: '428',
    OTP_EXPIRY: 300000,
    NEWS_JSON_URL: '',
    BOT_NAME: 'BIG MUNYA MINI',
    ...
    BUTTON_IMAGES: {
        ALIVE: 'https://files.catbox.moe/x2tlx7.jpg',
        MENU: 'https://files.catbox.moe/x2tlx7.jpg',
        OWNER: 'https://files.catbox.moe/x70enq.jpg',
        SONG: 'https://files.catbox.moe/x70enq.jpg',
        VIDEO: 'https://files.catbox.moe/x70enq.jpg'
    }
};
...
> *ʜᴇʟʟᴏ ʙʀᴏ 👋, ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ BIG MUNYA MINI ʙᴏᴛ 🤖, ᴀ ᴘᴏᴡᴇʀꜰᴜʟ ᴍᴜʟᴛɪ-ᴅᴇᴠɪᴄᴇ ᴡʜᴀᴛꜱᴀᴘᴘ ʙᴏᴛ.*

> ᴛᴀᴘ ᴀʟʟᴍᴇɴᴜ ᴛᴏ sᴇᴇ ᴀʟʟ ᴄᴏᴍᴍᴀɴᴅs

ℹ️ғᴏʀ ʙᴏᴛ ᴜᴘᴅᴀᴛᴇs  
https://whatsapp.com/channel/0029Vb67RqC0G0Xqn0YpAK2t
  ...
  `.trim();
    ...
    await socket.sendMessage(sender, {
        image: { url: "https://files.catbox.moe/x2tlx7.jpg" },
        caption: text,
        footer: " ©POWERED BY BIG MUNYA",
        buttons: buttons,
        headerType: 4
    });
    break;
...
case 'setting': {
    ...
    const buttons = [
        ...
        { buttonId: `${config.PREFIX}alive`, buttonText: { displayText: "♻️ Alive" }, type: 1 },
        { buttonId: `${config.PREFIX}ping`, buttonText: { displayText: "📡 Ping" }, type: 1 },
        { buttonId: `${config.PREFIX}owner`, buttonText: { displayText: "ℹ️ Owner Info" }, type: 1 },
...
case 'winfo': {
    try {
        ...
        let caption = `*📢 BIG MUNYA WhatsApp Channel Info*\n\n`;
...
case 'gdrive': {
    ...
    const BOT_NAME = '© ᴘᴏᴡᴇʀᴇᴅ ʙʏ BIG MUNYA';
...
//=======================================
 case 'ping': {
    ...
    let ping = await socket.sendMessage(sender, { text: '*_Pinging to BIG MUNYA MINI ʙᴏᴛ Module..._* ❗' });
...
case 'owner': {
    ...
    const vcard = 'BEGIN:VCARD\n'
        ...
        + 'ORG:BIG MUNYA\n'
        + 'TEL;+263778974934\n'
        + 'EMAIL:nyaudemunya@gmail.com\n'
        + 'END:VCARD';

    await socket.sendMessage(sender, {
        contacts: {
            displayName: "BIG MUNYA",
            contacts: [{ vcard }]
        }
    }, { quoted: msg });
    ...
    await socket.sendMessage(sender, {
          image: { url: 'https://files.catbox.moe/x70enq.jpg' },
        caption: '*👨‍💻 BIG MUNYA MINI*\n\n'
               + '*👤 Name:* BIG MUNYA\n'
               + '*📞 Number:* wa.me/+263 778974934\n'
               + '*📧 Email:* nyaudemunya@gmail.com\n',
        footer: '⚡ ᴘᴏᴡᴇʀᴇᴅ ʙʏ BIG MUNYA',
        buttons: [
            { buttonId: `${config.PREFIX}menu`, buttonText: { displayText: '📋 MENU' }, type: 1 },
...
🖼️ *MEDIA TOOLS*
🚀 news - Latest Updates
🌟 active - See bot activity
💥 boom - Boom (count)
👁️‍🗨 viewonce - Open view once
👀 winfo - Get channel info
⌛ about - Get person info
...
case 'repo': {
                    await socket.sendMessage(sender, {
                        image: { url: 'https://files.catbox.moe/x2tlx7.jpg' },
                        caption: `🌟 *BIG MUNYA MINI BOT REPOSITORY*\n\n🔗 *GitHub:* https://github.com/SNOWBIRD0074/Lady-Bella2\n\n🌟 *Features:*\n• Fast & Reliable\n• Easy to Use\n• Multiple Sessions\n\n> © *ᴘᴏᴡᴇʀᴇᴅ ʙʏ BIG MUNYA MINI*`
                    });
                    break;
                }
...
case 'mfdl':
case 'mediafire': {
    if (!text) return socket.sendMessage(sender, { text: `Contoh:\n.mediafire https://www.mediafire.com/file/xfk1u8yl4uqbizx/nulis.zip/file` });
    if (!/mediafire\.com/.test(text)) return socket.sendMessage(sender, { text: `❌ Link tidak valid. Hanya support *mediafire.com*` });
    ...
    try {
        ...
        const mimetype = require('mime-types').lookup(ext.toLowerCase()) || 'application/octet-stream';

        let filesize = raw.find('ul.details li:nth-child(1) span').text().trim();
        const uploaded = raw.find('ul.details li:nth-child(2) span').text().trim();
        const dl = $('a#downloadButton').attr('data-scrambled-url');

        if (!dl) return socket.sendMessage(sender, { text: '❌ Gagal mengambil link download.' });

        const decoded = Buffer.from(dl, 'base64').toString().trim();

        // Normalize filesize to MB
        const sizeNum = parseFloat(filesize.replace(',', '.'));
        const sizeInMB = filesize.toLowerCase().includes('gb') ? sizeNum * 1024 : sizeNum;
...
🗓️ *Upload:* ${uploaded}
        ...
        const contextInfo = {
            externalAdReply: {
                ...
                title: filename,
                body: filesize + ' | Uploaded: ' + uploaded,
                sourceUrl: decoded
            }
        };
        ...
        if (sizeInMB <= 100) {
            ...
            await socket.sendMessage(sender, {
                text: caption + `\n\n📥 File terlalu besar. Download di sini:\n${decoded}`,
                contextInfo
            });
        }
        ...
        socket.sendMessage(sender, { text: `❌ Terjadi kesalahan: ${err.message}` });
    }
